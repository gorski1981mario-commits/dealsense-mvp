...
async function run() {
  ...
  const r = await post("/api/v2/finance/mortgage/quote", req);
  assert.strictEqual(r.status, 200);
  assert.ok(r.body && r.body.ok === true);
  assert.ok(Array.isArray(r.body.offersTop3));
  assert.ok(r.body.offersTop3.length > 0);
    ...
    JSON.stringify(
      {
        ok: true,
        tests_total: 1,
        results: [{ name: "finance mortgage v2 base+delta", status: r.status, request: req, response: pickImportant(r.body) }],
      },
      null,
      2
    )
...
run().catch((e) => {
  console.error("Finance mortgage V2 test runner failed:", e && e.stack ? e.stack : e);
  process.exitCode = 1;
});
