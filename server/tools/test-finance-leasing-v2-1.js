...
async function run() {
  ...
  const r = await post("/api/v2/finance/leasing/quote", req);
    ...
    JSON.stringify(
      {
        ok: true,
        tests_total: 1,
        results: [{ name: "finance leasing v2 base+delta", status: r.status, request: req, response: pickImportant(r.body) }],
      },
      null,
      2
    )
...
run().catch((e) => {
  console.error("Finance leasing V2 test runner failed:", e && e.stack ? e.stack : e);
  process.exitCode = 1;
});
