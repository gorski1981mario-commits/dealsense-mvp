...
async function run() {
  ...
  const req = {
    ...
    pickupLocation: "AMS",
    dropoffLocation: "AMS",
    pickupAt: isoNowPlusDays(0),
    dropoffAt: isoNowPlusDays(7),
    carType: "compact",
    basePriceEur: 260,
  ...
  const r = await post("/api/car-rental/quote", req);
  assert.strictEqual(r.status, 200);
  assert.ok(r.body && r.body.ok === true);
  assert.strictEqual(r.body.module, "car_rental");
    ...
    JSON.stringify(
      {
        ok: true,
        tests_total: 1,
        results: [{ name: "module2 car rental quote", status: r.status, request: req, response: pickImportant(r.body) }],
      },
      null,
      2
    )
...
run().catch((e) => {
  console.error("Module2 car rental test runner failed:", e && e.stack ? e.stack : e);
  process.exitCode = 1;
});
