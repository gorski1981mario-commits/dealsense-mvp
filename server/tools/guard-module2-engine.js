...
function main() {
  const bypass = (() => {
    const v = String(process.env.GUARD_MODULE2_BYPASS || "").trim().toLowerCase();
    return v === "1" || v === "true";
  })();
  ...
  if (mismatches.length > 0) {
    console.error("Module2 engine guard failed: unexpected fingerprints");
    console.error(JSON.stringify({ mismatches }, null, 2));
    if (!bypass) {
      process.exitCode = 1;
      return;
    }
    console.error("[guard-module2-engine] Bypassing mismatches due to GUARD_MODULE2_BYPASS=1");
    return;
  }

  console.log("OK: Module2 engine fingerprints match expected baseline.");
}
