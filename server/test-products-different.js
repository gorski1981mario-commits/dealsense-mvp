/**
 * TEST RÓŻNYCH PRODUKTÓW FIZYCZNYCH - 3 INNE KATEGORIE
 * 
 * Testuje 3 zupełnie różne produkty:
 * 1. Samsung Galaxy S24 Ultra - €1,449 (smartphone)
 * 2. Sony WH-1000XM5 - €399 (headphones)
 * 3. Nintendo Switch OLED - €349 (gaming console)
 */
  ...
  try {
    ...
    offers.forEach((offer, idx) => {
      ...
      const isNl = seller.includes('.nl') || 
                   ['bol', 'coolblue', 'mediamarkt', 'alternate', 'belsimpel', 'conrad'].some(s => seller.includes(s));
      if (isNl) {
        analysis.nlShops++;
      }
    ...
    analysis.titles.forEach((t, i) => {
      ...
      console.log(`${i+1}. ${status} ${t.seller.padEnd(25)} €${t.price.toString().padStart(6)} (${type}) - ${t.title}`);
    });
...
async function runTests() {
  console.log('\n🚀 TESTING 3 DIFFERENT PRODUCTS');
  console.log('Categories: Smartphone, Audio, Gaming');
  console.log('Checking: Real prices, Correct products, NL shops\n');
