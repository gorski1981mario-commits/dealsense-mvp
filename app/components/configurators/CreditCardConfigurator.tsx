    ...
    return (
      <div>
        ...
        {[{name: '💳 ING Gold Card', price: '€0/jaar', rewards: 'Cashback 1% + Reisverzekering', rating: '⭐ 4.7/5', trust: '🛡️ 9/10', score: 'Score: 9.3', badge: 'BESTE DEAL', best: true}, {name: '💳 ABN AMRO Premium', price: '€45/jaar', rewards: 'Miles 2% + Lounge toegang', rating: '⭐ 4.5/5', trust: '🛡️ 9/10', score: 'Score: 8.9'}, {name: '💳 Rabobank Platinum', price: '€75/jaar', rewards: 'Cashback 2% + Concierge', rating: '⭐ 4.6/5', trust: '🛡️ 8/10', score: 'Score: 8.7'}].map((card, i) => (
          <div key={i} style={{ background: card.best ? '#E6F4EE' : '#F9FAFB', border: `2px solid ${card.best ? '#10b981' : '#E5E7EB'}`, borderRadius: '12px', padding: '16px', marginBottom: '12px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '10px' }}>
              <div style={{ fontSize: '16px', fontWeight: 600, color: '#111827' }}>{card.name}</div>
              <div style={{ fontSize: '18px', fontWeight: 700, color: '#10b981' }}>{card.price}</div>
            </div>
            ...
            {card.badge && <span style={{ display: 'inline-block', padding: '4px 10px', background: '#10b981', color: 'white', borderRadius: '6px', fontSize: '11px', fontWeight: 600, marginTop: '8px' }}>{card.badge}</span>}
          </div>
        ))}
        <div style={{ background: '#E6F4EE', border: '2px solid #10b981', borderRadius: '12px', padding: '20px', textAlign: 'center', margin: '20px 0' }}>
          ...
          <div style={{ fontSize: '32px', fontWeight: 700, color: '#10b981', margin: '12px 0' }}>€0</div>
    ...
    return (
      <div>
        ...
        <div style={{ background: '#E6F4EE', border: '2px solid #10b981', borderRadius: '12px', padding: '20px', textAlign: 'center', margin: '20px 0' }}>
          ...
          <button onClick={() => setView('unlocked')} style={{ width: '100%', padding: '14px', background: 'linear-gradient(135deg, #10b981 0%, #15803d 100%)', color: 'white', border: 'none', borderRadius: '10px', fontSize: '15px', fontWeight: 600, cursor: 'pointer', boxShadow: '0 4px 12px rgba(30, 127, 92, 0.3)' }}>Bekijk deals →</button>
        </div>
      </div>
    )
  }
    ...
    return (
      <div>
        ...
        {[{name: '💳 ING Gold Card', price: '€0/jaar', rewards: 'Cashback 1% + Reisverzekering', rating: '⭐ 4.7/5 (18.234 reviews)', trust: '🛡️ Betrouwbaar 9/10', badge: 'BESTE DEAL - GRATIS', url: 'https://www.ing.nl', best: true}, {name: '💳 ABN AMRO Premium', price: '€45/jaar', rewards: 'Miles 2% + Lounge toegang', rating: '⭐ 4.5/5 (12.891 reviews)', trust: '🛡️ Betrouwbaar 9/10', url: 'https://www.abnamro.nl'}, {name: '💳 Rabobank Platinum', price: '€75/jaar', rewards: 'Cashback 2% + Concierge', rating: '⭐ 4.6/5 (15.456 reviews)', trust: '🛡️ Betrouwbaar 8/10', url: 'https://www.rabobank.nl'}].map((card, i) => (
          <div key={i} style={{ background: card.best ? '#E6F4EE' : 'white', border: `2px solid ${card.best ? '#10b981' : '#E5E7EB'}`, borderRadius: '12px', padding: '16px', marginBottom: '16px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '10px' }}>
              <div style={{ fontSize: '16px', fontWeight: 600, color: '#111827' }}>{card.name}</div>
              <div style={{ fontSize: '18px', fontWeight: 700, color: '#10b981' }}>{card.price}</div>
            </div>
            ...
            {card.badge && <span style={{ display: 'inline-block', padding: '4px 10px', background: '#10b981', color: 'white', borderRadius: '6px', fontSize: '11px', fontWeight: 600, marginTop: '8px' }}>{card.badge}</span>}
            <button onClick={() => window.open(card.url, '_blank')} style={{ width: '100%', marginTop: '12px', padding: '10px', background: '#10b981', color: 'white', border: 'none', borderRadius: '8px', fontSize: '13px', fontWeight: 600, cursor: 'pointer' }}>🌐 Aanvragen</button>
          </div>
        ))}
      </div>
    )
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div style={{ marginBottom: '24px', paddingBottom: '20px', borderBottom: '1px solid #E5E7EB' }}>
          ...
          <div style={{ marginBottom: '14px' }}>
            ...
            <input type="number" min="500" max="15000" step="500" value={limit} onChange={(e) => { const val = parseInt(e.target.value); setLimit(val); validateAndMark('limit', val, (v) => v > 0); }} disabled={isLocked} placeholder="2000" style={{ width: '100%', padding: '10px 14px', border: `2px solid ${validFields.has('limit') ? '#10b981' : '#E5E7EB'}`, borderRadius: '10px', fontSize: '14px', fontWeight: 500, color: '#111827', background: isLocked ? '#F3F4F6' : (validFields.has('limit') ? '#E6F4EE' : 'white'), boxShadow: validFields.has('limit') ? '0 0 0 3px rgba(30, 127, 92, 0.1)' : 'none', cursor: isLocked ? 'not-allowed' : 'text', transition: 'all 0.2s' }} />
          </div>

          <div>
            <label style={{ display: 'block', fontSize: '13px', fontWeight: 600, color: '#374151', marginBottom: '6px' }}>Gebruik</label>
            <select value={usage} onChange={(e) => { const val = e.target.value; setUsage(val); validateAndMark('usage', val); }} disabled={isLocked} style={{ width: '100%', padding: '10px 14px', border: `2px solid ${validFields.has('usage') ? '#10b981' : '#E5E7EB'}`, borderRadius: '10px', fontSize: '14px', fontWeight: 500, color: '#111827', background: isLocked ? '#F3F4F6' : (validFields.has('usage') ? '#E6F4EE' : 'white'), boxShadow: validFields.has('usage') ? '0 0 0 3px rgba(30, 127, 92, 0.1)' : 'none', cursor: isLocked ? 'not-allowed' : 'pointer', transition: 'all 0.2s' }}>
        ...
        <div style={{ marginBottom: '24px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
            {[{value: travelInsurance, setter: setTravelInsurance, label: '✈️ Reisverzekering', desc: 'Gratis verzekering bij reizen'}, {value: purchaseProtection, setter: setPurchaseProtection, label: '🛡️ Aankoopbescherming', desc: 'Bescherming tegen schade/diefstal'}, {value: contactless, setter: setContactless, label: '📶 Contactloos betalen', desc: 'NFC/tap-to-pay functie'}, {value: secondCard, setter: setSecondCard, label: '👥 Tweede kaart', desc: 'Extra kaart voor partner/kind'}].map((item, i) => (
              <div key={i} onClick={() => !isLocked && item.setter(!item.value)} style={{ display: 'flex', alignItems: 'flex-start', gap: '8px', padding: '10px 12px', border: '2px solid #E5E7EB', borderRadius: '8px', cursor: isLocked ? 'not-allowed' : 'pointer', background: item.value ? '#E6F4EE' : (isLocked ? '#F3F4F6' : 'white'), borderColor: item.value ? '#10b981' : '#E5E7EB', opacity: isLocked ? 0.6 : 1 }}>
