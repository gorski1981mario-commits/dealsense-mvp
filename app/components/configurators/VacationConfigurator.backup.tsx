        ...
        <form onSubmit={handleSubmit}>
          ...
          <div style={{ marginBottom: '24px', paddingBottom: '20px', borderBottom: '1px solid #E5E7EB' }}>
            ...
            <div>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px', background: children > 0 ? '#E6F4EE' : '#F3F4F6', borderRadius: '10px', padding: '8px 12px', width: 'fit-content', border: children > 0 ? '2px solid #10b981' : '2px solid transparent', transition: 'all 0.2s' }}>
                ...
                <div style={{ marginTop: '10px', display: 'flex', flexDirection: 'column', gap: '6px' }}>
                  {childrenAges.map((age, i) => (
                    <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                      ...
                      <select value={age || 1} onChange={(e) => { if (!isLocked) { const newAges = [...childrenAges]; newAges[i] = parseInt(e.target.value); setChildrenAges(newAges); const allValid = newAges.every(a => a >= 1 && a <= 17); if (allValid) { validateAndMark('childrenAges', newAges, () => true); } else { validFields.delete('childrenAges'); }}}} onFocus={() => setActiveField(`childAge${i}`)} onBlur={() => setActiveField(null)} disabled={isLocked} style={{ flex: 1, padding: '8px 10px', border: age >= 1 ? '2px solid #10b981' : '2px solid #E5E7EB', borderRadius: '8px', fontSize: '13px', background: isLocked ? '#F3F4F6' : (age >= 1 ? '#E6F4EE' : 'white'), cursor: isLocked ? 'not-allowed' : 'pointer', transition: 'all 0.2s' }}>
          ...
          <div style={{ marginBottom: '24px', paddingBottom: '20px', borderBottom: '1px solid #E5E7EB' }}>
            ...
            <div style={{ marginBottom: '14px' }}>
              ...
              <select value={destination} onChange={(e) => { const val = e.target.value; setDestination(val); validateAndMark('destination', val); }} onFocus={() => setActiveField('destination')} onBlur={() => setActiveField(null)} disabled={isLocked} style={{ width: '100%', padding: '10px 14px', border: validFields.has('destination') ? '2px solid #10b981' : (touchedFields.has('destination') ? '2px solid #F59E0B' : '2px solid #E5E7EB'), borderRadius: '10px', fontSize: '14px', fontWeight: 500, color: '#111827', background: isLocked ? '#F3F4F6' : (validFields.has('destination') ? '#E6F4EE' : (touchedFields.has('destination') ? '#FEF3C7' : 'white')), cursor: isLocked ? 'not-allowed' : 'pointer', transition: 'all 0.2s' }}>
            ...
            <div style={{ marginBottom: '14px' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                {[{value: 'hotel', label: '🏨 Hotel'}, {value: 'apartment', label: '🏠 Appartement'}, {value: 'resort', label: '🏖️ Resort'}, {value: 'park', label: '🏡 Vakantiepark'}].map(a => (
                  <div key={a.value} onClick={() => { if (!isLocked) { setAccommodationType(a.value); validateAndMark('accommodationType', a.value); } }} onFocus={() => setActiveField('accommodationType')} onBlur={() => setActiveField(null)} tabIndex={0} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '10px 12px', border: accommodationType === a.value ? '2px solid #10b981' : '2px solid #E5E7EB', borderRadius: '8px', cursor: isLocked ? 'not-allowed' : 'pointer', background: accommodationType === a.value ? '#E6F4EE' : (isLocked ? '#F3F4F6' : 'white'), opacity: isLocked ? 0.6 : 1, transition: 'all 0.2s' }}>
            ...
            <div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                {[{value: '3', label: '⭐⭐⭐ 3 sterren'}, {value: '4', label: '⭐⭐⭐⭐ 4 sterren'}, {value: '5', label: '⭐⭐⭐⭐⭐ 5 sterren'}].map(s => (
                  <div key={s.value} onClick={() => !isLocked && selectStar(s.value)} onFocus={() => setActiveField('stars')} onBlur={() => setActiveField(null)} tabIndex={0} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '10px 12px', border: stars === s.value ? '2px solid #10b981' : '2px solid #E5E7EB', borderRadius: '8px', cursor: isLocked ? 'not-allowed' : 'pointer', background: stars === s.value ? '#E6F4EE' : (isLocked ? '#F3F4F6' : 'white'), opacity: isLocked ? 0.6 : 1, transition: 'all 0.2s' }}>
          ...
          <div style={{ marginBottom: '24px', paddingBottom: '20px', borderBottom: '1px solid #E5E7EB' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
              {[{value: 'bb', label: '🍳 Logies & Ontbijt'}, {value: 'hb', label: '🍽️ Halfpension'}, {value: 'fb', label: '🍽️🍽️ Volpension'}, {value: 'ai', label: '🍹 All Inclusive'}, {value: 'uai', label: '🍹+ Ultra All Inclusive'}].map(b => (
                <div key={b.value} onClick={() => { if (!isLocked) { setBoard(b.value); validateAndMark('board', b.value); } }} onFocus={() => setActiveField('board')} onBlur={() => setActiveField(null)} tabIndex={0} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '10px 12px', border: board === b.value ? '2px solid #10b981' : (touchedFields.has('board') && !validFields.has('board') ? '2px solid #F59E0B' : '2px solid #E5E7EB'), borderRadius: '8px', cursor: isLocked ? 'not-allowed' : 'pointer', background: board === b.value ? '#E6F4EE' : (isLocked ? '#F3F4F6' : (touchedFields.has('board') && !validFields.has('board') ? '#FEF3C7' : 'white')), opacity: isLocked ? 0.6 : 1, transition: 'all 0.2s' }}>
          ...
          <div style={{ marginBottom: '24px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '10px' }}>
              {[{value: 'pool', label: '🏊 Zwembad'}, {value: 'wifi', label: '📶 Wifi'}, {value: 'parking', label: '🅿️ Parkeren'}, {value: 'kids', label: '👶 Kinderclub'}, {value: 'spa', label: '💆 Spa/Wellness'}, {value: 'beach', label: '🏖️ Strand'}, ...(children === 0 ? [{value: 'adults', label: '🔞 Adults Only'}] : [])].map(e => (
                <div key={e.value} onClick={() => !isLocked && toggleExtra(e.value)} tabIndex={0} style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '8px 10px', border: extras.includes(e.value) ? '2px solid #10b981' : '2px solid #E5E7EB', borderRadius: '8px', cursor: isLocked ? 'not-allowed' : 'pointer', background: extras.includes(e.value) ? '#E6F4EE' : (isLocked ? '#F3F4F6' : 'white'), opacity: isLocked ? 0.6 : 1, transition: 'all 0.2s' }}>
    ...
    return (
      <div>
        ...
        {[{name: '⭐⭐⭐⭐⭐ Delphin Imperial', price: '€1.749', location: '📍 Lara, Turkije • Ultra All Inclusive', rating: '⭐ 4.8/5', trust: '🛡️ 9/10', score: 'Score: 9.2', badge: 'BESTE DEAL', best: true}, {name: '⭐⭐⭐⭐⭐ Rixos Premium Belek', price: '€1.899', location: '📍 Belek, Turkije • All Inclusive', rating: '⭐ 4.5/5', trust: '🛡️ 9/10', score: 'Score: 8.8'}, {name: '⭐⭐⭐⭐⭐ Maxx Royal Belek', price: '€2.299', location: '📍 Belek, Turkije • Ultra All Inclusive', rating: '⭐ 4.7/5', trust: '🛡️ 10/10', score: 'Score: 9.0'}].map((hotel, i) => (
          <div key={i} style={{ background: hotel.best ? '#E6F4EE' : '#F9FAFB', border: `2px solid ${hotel.best ? '#10b981' : '#E5E7EB'}`, borderRadius: '12px', padding: '16px', marginBottom: '12px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '10px' }}>
              <div style={{ fontSize: '16px', fontWeight: 600, color: '#111827' }}>{hotel.name}</div>
              <div style={{ fontSize: '18px', fontWeight: 700, color: '#10b981' }}>{hotel.price}</div>
            </div>
            ...
            {hotel.badge && <span style={{ display: 'inline-block', padding: '4px 10px', background: '#10b981', color: 'white', borderRadius: '6px', fontSize: '11px', fontWeight: 600, marginTop: '8px' }}>{hotel.badge}</span>}
          </div>
        ))}
        ...
        <div style={{ background: '#E6F4EE', border: '2px solid #10b981', borderRadius: '12px', padding: '20px', textAlign: 'center', margin: '20px 0' }}>
          ...
          <div style={{ fontSize: '32px', fontWeight: 700, color: '#10b981', margin: '12px 0' }}>€174,90</div>
  ...
  return (
    <div>
      ...
      {[{name: '⭐⭐⭐⭐⭐ Delphin Imperial', price: '€1.749 p.p.', location: '📍 Lara, Turkije • Ultra All Inclusive', rating: '⭐ 4.8/5 (2.341 reviews)', trust: '🛡️ Betrouwbaar 9/10', badge: 'BESTE DEAL - BESPAAR €450', url: 'https://www.corendon.nl', best: true}, {name: '⭐⭐⭐⭐⭐ Rixos Premium Belek', price: '€1.899 p.p.', location: '📍 Belek, Turkije • All Inclusive', rating: '⭐ 4.5/5 (1.892 reviews)', trust: '🛡️ Betrouwbaar 9/10', url: 'https://www.tui.nl'}, {name: '⭐⭐⭐⭐⭐ Maxx Royal Belek', price: '€2.299 p.p.', location: '📍 Belek, Turkije • Ultra All Inclusive', rating: '⭐ 4.7/5 (3.127 reviews)', trust: '🛡️ Betrouwbaar 10/10', url: 'https://www.sunweb.nl'}].map((hotel, i) => (
        <div key={i} style={{ background: hotel.best ? '#E6F4EE' : 'white', border: `2px solid ${hotel.best ? '#10b981' : '#E5E7EB'}`, borderRadius: '12px', padding: '16px', marginBottom: '16px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '10px' }}>
            <div style={{ fontSize: '16px', fontWeight: 600, color: '#111827' }}>{hotel.name}</div>
            <div style={{ fontSize: '18px', fontWeight: 700, color: '#10b981' }}>{hotel.price}</div>
          </div>
          ...
          <div style={{ display: 'flex', gap: '10px', marginTop: '12px' }}>
            <button onClick={() => window.open(hotel.url, '_blank')} style={{ flex: 1, padding: '10px', background: '#10b981', color: 'white', border: 'none', borderRadius: '8px', fontSize: '13px', fontWeight: 600, cursor: 'pointer' }}>🌐 Boek bij {hotel.url.includes('corendon') ? 'Corendon' : hotel.url.includes('tui') ? 'TUI' : 'Sunweb'}</button>
