...
function loadManifest(root) {
  const manifestPath = path.join(root, "tools", "finance_engine_manifest.json");
  const raw = fs.readFileSync(manifestPath, "utf8");
  const manifest = JSON.parse(raw);
  const files = (manifest && manifest.files) || {};
  const rels = Object.keys(files);
  if (rels.length === 0) throw new Error("finance_engine_manifest.json has no files");
...
function main() {
  ...
  const name = String(process.env.FINANCE_SNAPSHOT_NAME || "finance-stable").trim() || "finance-stable";
