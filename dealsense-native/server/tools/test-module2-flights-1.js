...
async function run() {
  ...
  const r = await post("/api/flights/quote", req);
  assert.strictEqual(r.status, 200);
  assert.ok(r.body && r.body.ok === true);
  assert.strictEqual(r.body.module, "flights");
    ...
    JSON.stringify(
      {
        ok: true,
        tests_total: 1,
        results: [{ name: "module2 flights quote", status: r.status, request: req, response: pickImportant(r.body) }],
      },
      null,
      2
    )
...
run().catch((e) => {
  console.error("Module2 flights test runner failed:", e && e.stack ? e.stack : e);
  process.exitCode = 1;
});
