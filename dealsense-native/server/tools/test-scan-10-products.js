...
async function runOne({ url, timeoutMs, product, i }) {
  const fingerprint = `test_scan10_${Date.now()}_${i}_${Math.random().toString(16).slice(2)}`;
  ...
  return {
    ...
    sample:
      data && typeof data === "object"
        ? pick(data, ["savings", "marketAvg", "dealsenseFee", "netToUser", "usage_count", "planId", "paidScansRemaining"])
        : null,
  };
}
...
main().catch((err) => {
  console.error("test-scan-10-products failed:", (err && err.message) || err);
  process.exitCode = 1;
});
