  ...
  <head>
    ...
    <style>
      ...
      * { box-sizing: border-box; }
      ...
      tbody td { font-size: 12px; padding: 4px 6px; border: 1px solid var(--grid); vertical-align: middle; }
...
function drawBoxTable(headers, rows) {
  ...
  for (const r of rows) {
    for (let i = 0; i < cols; i += 1) {
      const cell = toCell(r[i]);
      widths[i] = Math.max(widths[i], cellLen(cell));
    }
  }
...
async function main() {
  ...
  const products = productSets[setName] || productSets.v1;
