...
async function getTop3EchoOffers(input) {
  ...
  const base_seller = input.base_seller || null; // Nowe: sklep z ceny bazowej
  ...
  // Filtr baseSeller: nie pokazuj tego samego sklepu co cena bazowa
  const withoutBaseSeller = base_seller ? normalized.filter(o => {
    const seller = (o.seller || '').toLowerCase();
    const baseSellerLower = base_seller.toLowerCase();
    return !seller.includes(baseSellerLower) && !baseSellerLower.includes(seller);
  }) : normalized;

  const marketAvg = computeMarketAvg(withoutBaseSeller, base_price);

  const annotated = withoutBaseSeller.map((o) => {
  ...
  // Oblicz medianę z ofert jako punkt odniesienia
  const prices = cleaned.map(o => o.price).filter(p => typeof p === "number" && Number.isFinite(p) && p > 0);
  const medianPrice = prices.length > 0 ? prices.sort((a, b) => a - b)[Math.floor(prices.length / 2)] : base_price;
  
  const savings = best && typeof best.price === "number" ? round2(medianPrice - best.price) : 0;

  const meta = {
    inputOffers: withoutBaseSeller.length,
