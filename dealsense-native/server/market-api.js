...
const USE_MOCK_FALLBACK = (process.env.USE_MOCK_FALLBACK || "false").trim().toLowerCase() !== "false"; // Fallback do mock danych jeśli API nie działa
...
function qualityConfig() {
  ...
  const blockedKeywords = [
    // Akcesoria
    "hoes", "hoesje", "bandje", "filter", "stofzak", "tas", "oplader", "kabel", "screenprotector", "oordopjes",
    "case", "kit", "module", "lens", "armor", "fabric", "display", "pro kit", "cine pro",
    // Abonamenty
    "abonnement", "kpn", "mobiel abonnement", "onbeperkt", "50plusmobiel",
    // Używane NL
    "gebruikt", "tweedehands", "2e hands", "gereviseerd", "gerenoveerd", "nieuwstaat", "als nieuw", "zo goed als nieuw", "occasion", "retour", "demo", "demonstratie", "showmodel", "displaymodel", "uitpakmodel", "open doos", "geopend", "beschadigd",
    // Używane EN
    "used", "second hand", "refurbished", "pre-owned", "renewed", "reconditioned", "open box", "returned", "like new"
  ];

  return { enabled, maxDeliveryDays, blockUsed, blockedKeywords };
}
...
function looksUsedOrMarketplace(offer, qcfg) {
  ...
  // Blocked keywords check (akcesoria, używane, itp.)
  if (qcfg.blockedKeywords && Array.isArray(qcfg.blockedKeywords)) {
    for (const kw of qcfg.blockedKeywords) {
      if (kw && txt.includes(kw.toLowerCase())) return true;
    }
  }
  ...
  try {
    ...
    for (const p of providerChain) {
      ...
      if (p === "serpapi") {
        ...
        console.log(`[fetchMarketOffers] API RETURNED (serpapi): ${merged ? merged.length : 0} offers`);
      } else {
        ...
        offersFromProvider = merged;
        console.log(`[fetchMarketOffers] API RETURNED (searchapi): ${merged ? merged.length : 0} offers`);
      }
    ...
    console.log(`[fetchMarketOffers] AFTER API: ${offersFromProvider.length} offers`);

    let offers = filterNlRetailOnly(filterBlockedOffers(offersFromProvider));
    console.log(`[fetchMarketOffers] AFTER NL/BLOCKED FILTERS: ${offers.length} offers`);
...
async function fetchMarketOffers(productName, ean = null) {
  ...
  console.log(`[fetchMarketOffers] START: productName="${productName}", ean="${ean}"`);
  console.log(`[fetchMarketOffers] USE_MOCK_FALLBACK=${USE_MOCK_FALLBACK}`);
  ...
  if (!CACHE_BYPASS) {
    ...
    if (cached) {
      ...
      console.log(`[fetchMarketOffers] CACHE HIT: ${cached.length} offers`);
      const filtered = filterNlRetailOnly(filterBlockedOffers(cached));
      console.log(`[fetchMarketOffers] CACHE FILTERED: ${filtered.length} offers`);
